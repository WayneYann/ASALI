...
namespace ASALI
{
    class GROWgrid 
    {
        ...
        private:
            ...
            OpenSMOKE::OpenSMOKEVectorDouble addTh_;
            OpenSMOKE::OpenSMOKEVectorDouble addTp_;
            OpenSMOKE::OpenSMOKEVectorDouble interpolatedTbulk_;
            OpenSMOKE::OpenSMOKEVectorDouble interpolatedTh_;
            OpenSMOKE::OpenSMOKEVectorDouble interpolatedTp_;
    ...
    void GROWgrid::calculateErrors()
    {
        ...
        weightedError(Th_, errors_);
        for ( unsigned int k=0;k<NP_;k++)
            errorMatrix_[k][NC_+NC_+SURF_NC_+1+1]  = errors_[k+1];

        weightedError(Tp_, errors_);
        for ( unsigned int k=0;k<NP_;k++)
            errorMatrix_[k][NC_+NC_+SURF_NC_+1+1+1] = errors_[k+1];
    ...
    void GROWgrid::addition()
    {
        ...
        ChangeDimensions(addN_, &addTh_, true);
        ChangeDimensions(addN_, &addTp_, true);
        ...
        for (unsigned int k=0;k<NP_;k++)
        {
            if (errorBool_[k+1] == true)
            {
                ...
                addTh_[counter+1]    = (Th_[k+1] + Th_[k+1+1])*0.5;
                addTp_[counter+1]    = (Tp_[k+1] + Tp_[k+1+1])*0.5;
    ...
    void GROWgrid::interpolation()
    {
        ...
        ChangeDimensions(interpolatedNP_, &interpolatedTh_, true);
        ...
        insertion(Th_,addTh_,interpolatedTh_);
        insertion(Tp_,addTp_,interpolatedTp_);
        ...
        for (unsigned int i=0;i<interpolatedNP_;i++)
        {
            ...
            interpolatedVariables_[counter++] = interpolatedTh_[i+1];
            interpolatedVariables_[counter++] = interpolatedTp_[i+1];
        }
    }
