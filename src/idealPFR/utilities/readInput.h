...
namespace ASALI
{
	class READinput 
	{
		public:
			...
			double getSectionArea()         const  { return As_;};
			double getSpecificArea()        const  { return 4./Dh_;};
			double getHydraulicDiameter()   const  { return Dh_;};
			...
			std::string getReactorType()    const  { return type_;};
	...
	void READinput::reactor()
	{
		...
		else
		{
			...
			words[0] = "type";
			words[1] = "length";
			words[2] = "inlet section area";
			words[3] = "hydraulic diameter";

			for (unsigned int i=0;i<dummyVector.size();i++)
			{
				if 		(dummyVector[i] == "type") 				{checkWord[0] = true; typeIndex        = i;}
				else if (dummyVector[i] == "length") 			{checkWord[1] = true; lengthIndex      = i;}
				else if (dummyVector[i] == "inlet" &&
						 dummyVector[i+1] == "section" && 
						 dummyVector[i+2] == "area" )			{checkWord[2] = true; areaIndex        = i;}
				else if (dummyVector[i] == "hydraulic" &&
						 dummyVector[i+1] == "diameter")		{checkWord[3] = true; dhIndex           = i;}
			}
			...
			{
				As_ = boost::lexical_cast<double>(dummyVector[areaIndex+3]);
				std::string dim = dummyVector[areaIndex+4];
				ConvertsToMeterSquare(As_, dim);
			}

			{
				Dh_ = boost::lexical_cast<double>(dummyVector[dhIndex+2]);
				std::string dim = dummyVector[dhIndex+3];
				ConvertsToMeter(Dh_, dim);
			}
	...
	void READinput::recapOnScreen()
	{
		...
		std::cout << "Inlet section area:                      = " << As_ << "\t[m2]" << std::endl;
		std::cout << "Hydraulic diameter                       = " << Dh_ << "\t[m]" << std::endl;
		std::cout << "\n################################################################################################" << std::endl;
