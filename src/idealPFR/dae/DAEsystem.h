...
void DAESystem::setInert(const std::string inert)
{
	inert_		= inert;
}

void DAESystem::setReactorGeometry(const double alfa, const double av, const double G, const double Dh)
{
	alfa_			= alfa;
	G_				= G;
	av_				= av;
...
void DAESystem::MassTransferCoefficient(const double z)
{
...
void DAESystem::AlgebraicEquations(BzzVectorInt& algebraic)
{
	...
	unsigned int counter = 1;
	{
		...
		{
			if ( thermodynamicsMapXML->NamesOfSpecies()[j-1] != "Rh(s)" )
			{
				algebraic[counter++] = 1;
			}
			else
