...
namespace ASALI
{
	class READinput 
	{
		public:
			...
			std::string getLimitantReactant()          const  { return limitant_;};
			std::string getInertSpecie()               const  { return inert_;};

			std::vector<double> getStoichiometryCoefficient()     const  { return stoichValue_; };
			std::vector<double> getFeedComposition()              const  { return inletValue_; };

			std::vector<std::string> getStoichiometryName()       const  { return stoichName_; };
		...
		private:
			...
			std::vector<std::string> stoichName_;
			std::vector<std::string> inputVector_;
			
			std::vector<double>      inletValue_;
			std::vector<double>      stoichValue_;
	...
	void READinput::reaction()
	{
		...
		else
		{
			...
			unsigned int stoichIndex     = 0;
			unsigned int inertIndex      = 0;
			unsigned int limitantIndex   = 0;
			...
			words[1] = "stoichiometry";
			words[2] = "inert specie";
			words[3] = "limitant reactant";
			words[4] = "results";
			...
			for (unsigned int i=0;i<dummyVector.size();i++)
			{
				if		(dummyVector[i] == "mole" &&
						 ...
						 dummyVector[i+1] == "fraction") 			{checkWord[0] = true; fractionIndex    = i;}
				else if (dummyVector[i] == "stoichiometry") 		{checkWord[1] = true; stoichIndex      = i;}
				else if (dummyVector[i] == "inert" &&
						 dummyVector[i+1] == "specie")				{checkWord[2] = true; inertIndex       = i;}
				else if (dummyVector[i] == "limitant" &&
						 dummyVector[i+1] == "reactant")			{checkWord[3] = true; limitantIndex    = i;}
				else if (dummyVector[i] == "results") 				{checkWord[4] = true; resultsIndex     = i;}
			}
			...
			limitant_ = dummyVector[limitantIndex + 2];
			...
			if ( dummyVector[stoichIndex+1] != "(" )
			{
				error();
				std::cout << "The Reaction properties/stoichiometry sub-dictionary must start with (\n" << std::endl;
				exit (EXIT_FAILURE);
			}
			else
			{
				unsigned int finalCount = 1e05;
				for (unsigned int i=stoichIndex;i<dummyVector.size();i++)
				{
					if (dummyVector[i] == ")")
					{
						finalCount = i;
				...
				{
					error();
					std::cout << "The Reaction properties/stoichiometry sub-dictionary must finish with )\n" << std::endl;
					exit (EXIT_FAILURE);
				}

				unsigned int k=0;
				for (unsigned int i=stoichIndex+1+1;i<finalCount;i++)
				{
					if (dummyVector[i] == "(")
					{
						error();
					...
					{
						stoichValue_.resize(k+1);
						stoichName_.resize(k+1);
						stoichValue_[k] = boost::lexical_cast<double>(dummyVector[i+1]);
						stoichName_[k]  = dummyVector[i];
						k++;
						i++;
					}
				}
